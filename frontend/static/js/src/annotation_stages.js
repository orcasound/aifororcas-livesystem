...
/*
 * Purpose:
 *   Load candidate annotations from a file, delete a few, undelete a few, add a few and submit
 * Dependencies:
 *   jQuery, audio-annotator.css, Wavesurfer (lib/wavesurfer.js), Message (src/message.js)
 */

function AnnotationReveal(wavesurfer) {
    this.currentStage = 0;
    this.currentRegion = null;
    this.stageOneView = new StageOneView();
    this.stageTwoView = new StageTwoView();
...
AnnotationReveal.prototype = {
    ...
    trackBeginingOfRegionCreation: function(region) {
        // this.trackEvent('start-to-create', region.id);
        $(region.element).addClass('new_region');
        $(region.annotationLabel.element).addClass('new_label');
    },
