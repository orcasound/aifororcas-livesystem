...
import json
import os
from DateRangeHLSStream import DateRangeHLSStream
...
def write_annotations_to_tsv(tsv_full_filename, clip_name, local_confidences, annotation_threshold):
    ...
    with open(tsv_full_filename, "a") as f:
        ...
        for i in range(num_predictions):
            prediction_start_time = i*DURATION_S
...
def create_prediction_explorer_dataset(start_time_str, end_time_str, s3_stream, model_path, annotation_threshold, round_id, dataset_folder):
    ...
    start_time_unix = int(time.mktime(start_dt_aware.timetuple()))

    # 2020-07-25 20:15
    end_dt = datetime.strptime(end_time_str, '%Y-%m-%d %H:%M')
    end_dt_aware = timezone('US/Pacific').localize(end_dt)
    end_time_unix = int(time.mktime(end_dt_aware.timetuple()))
...
def main():
    ...
    create_prediction_explorer_dataset(args.start_time, args.end_time, args.s3_stream, args.model_path, args.annotation_threshold, args.round_id, args.dataset_folder)
